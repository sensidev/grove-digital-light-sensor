...
uint8_t GroveTSL2561::readRegister(int reg)
{
    char reg_buf[1];
    char data_read_buf[1];
...
void GroveTSL2561::writeRegister(int reg, uint8_t val)
{
    char reg_buf[1];
...
{
    CH0_LOW=readRegister(TSL2561_Channal0L);
    CH0_HIGH=readRegister(TSL2561_Channal0H);

    //read two bytes from registers 0x0E and 0x0F
    CH1_LOW=readRegister(TSL2561_Channal1L);
    CH1_HIGH=readRegister(TSL2561_Channal1H);
...
signed long GroveTSL2561::readVisibleLux()
{
    writeRegister(TSL2561_Control, 0x03);  // POWER UP
    wait_ms(14);
    getLux();
...
unsigned long GroveTSL2561::calculateLux(unsigned int iGain, unsigned int tInt,int iType)
{
    switch (tInt)
    {
        case 0:  // 13.7 msec
